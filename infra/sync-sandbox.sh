#!/bin/sh
# Sync local sandbox directory to the Azure Files 'sandbox' share
# This uploads the bot's sandbox working directory content
# to the file share mounted at /sandbox on the function app.
#
# Usage:
#   infra/sync-sandbox.sh [-d|--destructive]
#
...
while [ $# -gt 0 ]; do
  case "$1" in
    -d|--destructive)
      ...
      ;;
    -h|--help)
      echo "Usage: infra/sync-sandbox.sh [-d|--destructive]"
      echo ""
      echo "Syncs sandbox/ to the 'sandbox' Azure Files share."
...
DEST_URL="https://${STORAGE_ACCOUNT}.file.core.windows.net/${SHARE_NAME}?${SAS}"

if [ "$DESTRUCTIVE" = 1 ]; then
  echo "📤 Syncing sandbox to 'sandbox' share..."
  echo "   Source: $SOURCE_DIR"
  echo "   Dest:   https://${STORAGE_ACCOUNT}.file.core.windows.net/${SHARE_NAME}"
